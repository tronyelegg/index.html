...
IMAGE_DIR = "images" # 💡 이미지를 저장할 로컬 폴더 이름
...
def main():
    ...
    print("🛠️ 3. 데이터 파싱 및 봇(Bot) 이미지 다운로드 시작...")
    new_cnt, upd_cnt = 0, 0
    ...
    for item in items:
        ...
        res_id = item.get("resource_id") # 💡 리소스 ID 추출
        ...
        main_db[code].update({
            ...
            "resource_id": res_id # 💡 DB에 리소스 ID 저장 완료!
        })
        ...
        if res_id:
            ...
            # 내 폴더에 이미지가 없다면 공식 서버에서 즉시 다운로드하여 저장
            if not os.path.exists(img_path):
                ...
                try:
                    ...
                    if img_resp.status_code == 200:
                        ...
                        print(f"🖼️ 신규 이미지 아카이빙 완료: {code}.png")
                except Exception:
                    pass
